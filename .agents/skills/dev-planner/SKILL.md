---
name: dev-planner
description: 当 Product-Spec.md 和架构文档已完成、需要规划怎么分阶段开发时使用。也在 Spec 或架构变更后用于更新已有开发计划。输出 DEV-PLAN.md。
version: 2.1
depends_on:
...
[任务与边界]
    ...
    - DEV-PLAN.md 明确继承 ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR 中的层次边界、目录职责和技术决策
...
[第一性原则]
    ...
    5. 架构约束优先：Phase 拆分必须服从架构文档的层次边界、目录职责、当前范围 / 后续范围和 ADR 决策

[依赖检测]
    依赖项：
    - Product-Spec.md  —— 必需
    - ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR —— 必需（路径可由 caller 指定）
    ...
    - 架构文档缺失 → 终止："需要架构文档，未找到 ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR。"
...
[分析维度]
    必须分析（不达标不能生成 Plan）：
    - 架构边界：读取 ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR，提取层次边界、目录职责、当前创建范围、后续占位范围、禁止跨层规则、ADR 技术决策。
    ...
    - 架构约束映射：每个 Phase 标明遵守的目录职责 / 层次边界 / ADR 决策；不能把后续范围写成当前交付物。

    尽量分析（不足不写占位符）：
    - 数据库设计：表名 + 首次创建 Phase + 用途
    - Phase 验收标准：能编译 + 能启动 + 新功能可用
    - 已知风险与限制：技术风险或已知限制
    - 信息不足但不阻塞规划 → 写入"已知风险与限制"，说明缺口和后续确认点
    - 信息不足且会影响 Phase 拆分 / 技术栈 / 架构边界 → 走 [确认策略]，不生成含占位符的 DEV-PLAN.md
...
[工作流程（生成模式）]
    1. 加载基准：
       ...
       - 读 Product-Spec.md → 提取产品类型 / 核心功能 / 辅助功能 / 技术方向 / UI 布局 / 数据存储
         （含未决标记或信息缺口 → 列出给用户确认，不写入最终 DEV-PLAN.md 作为占位）
       - 读 ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR → 提取架构边界、目录职责、当前范围、后续范围、non-goals、ADR 决策
    ...
    3. 分析（产出 Phase 草案）：
       ...
       - 架构约束映射：把功能映射到入口层 / 核心层 / 共享协议 / 适配层及具体目录
...
[工作流程（迭代模式）]
    触发：已有 DEV-PLAN.md + Spec 变更 / 架构文档变更 / 用户主动要求调整

    1. 加载现有状态：读 DEV-PLAN.md（识别 ✅ Phase）+ 更新后 Product-Spec.md + Product-Spec-CHANGELOG.md（如有）+ ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR + Brief / 设计稿同步检查
    ...
    2. 按变更类型识别影响：
       ...
       - 架构边界变更 → 重新校验 Phase 目录职责、依赖顺序、当前范围 / 后续范围；已完成 Phase 不自动改，只标注返工风险
...
[初始化]
    ...
    3. 路由：
       ...
       - 有 DEV-PLAN.md + Spec / 架构变更 → [工作流程（迭代模式）]
