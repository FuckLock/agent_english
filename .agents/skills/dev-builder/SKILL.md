...
[第一性原则]
    ...
    4. 架构边界优先：编码必须服从 architecture_context 中的层次边界、目录职责、当前范围 / 后续范围、ADR 决策；不能为了快把核心逻辑塞进入口层或把未来范围提前实现。

    5. 文件精简：单文件 ≤ 300 行（超了按职责拆分）；三行简单代码好过一个过度抽象
...
[依赖检测]
    ...
    - ARCHITECTURE.md / PROJECT-STRUCTURE.md / ADR —— 必需（caller 可传自定义路径）
    - architecture_context —— 必需（由 caller 从架构文档提取后传给 generator / 开发执行方）
    ...
    - 必需缺失（Spec / 架构文档 / architecture_context / Plan / 技术栈工具）→ 终止："dev-builder 需要 [依赖名]，未找到。"
...
[开发规则]
    ...
    【架构层面】
    - 入口层只放 UI / 命令 / 服务入口、运行容器适配、平台能力和导航组合。
    - 核心业务规则、领域模型、状态机、共享算法不得写进入口层。
    - 适配层只处理外部 API / SDK / 存储 / 系统能力转换，不反向污染核心模型。
    - 共享协议 / contracts 里的类型、事件、错误状态必须稳定命名，不夹带平台 UI 细节。
    - 当前 Phase 只能创建 architecture_context 标为当前范围的目录 / 模块；后续占位范围不得提前实现。
    - 如交付清单与 architecture_context 冲突，停止并返回 blocked / needs_context；不自行改架构。
    ...
    约束原则：具体目录职责以 PROJECT-STRUCTURE.md / architecture_context 为准；不允许凭个人偏好新增平行目录。
...
[Phase 完成度判断]
    ...
    3. 质量门槛 + 架构边界：每个功能通过 [开发规则].【质量门槛】6 项，并逐项确认文件路径、依赖方向、模块职责没有违反 architecture_context
...
[工作流程]
    ...
    第 1 步：前置检查
        ...
        注：本 skill 是 library，不直接读取架构文档；由 caller / generator 读取 ARCHITECTURE.md、PROJECT-STRUCTURE.md、ADR 后传入 architecture_context，再引用本 skill 的规则执行。
